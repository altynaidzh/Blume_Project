...
async function renderGoods() {
  ...
  section.innerHTML = "";
  data.forEach(({ price, title, description, img, id }) => {
    section.innerHTML += `
            <div class="card m-4 cardBook" style="width: 20rem">
                <img id="${id}" src=${img} class="card-img-top detailsCard" style="heigth: 280px" alt="${title}"/>
                <div class="card-body">
...
document.addEventListener("click", async ({ target: { classList, id } }) => {
  ...
  if (classes.includes("btnEdit")) {
    ...
    const { title, description, img, price, id: productId } = await res.json();
